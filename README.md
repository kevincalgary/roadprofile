...
- **Corrections & moderation** — VIN/spec correction workflow, duplicate-vehicle merge tool, report queue, warn/suspend/ban, content removal with history, audit log, and an appeals workflow (a user appeals a warn/suspend/ban/content-removal taken against them; a moderator upholds or overturns it, with an overturn automatically reversing the underlying suspension/ban/removal) — all gated behind a server-enforced moderator role the client can never self-assign.
